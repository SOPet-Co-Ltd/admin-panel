...
import { ArrowUpTray, EllipsisHorizontal, Plus, Trash } from '@medusajs/icons';
import {
  Button,
  Checkbox,
  Container,
...
export const CustomTagRequest = () => {
  ...
  const currentType = useMemo(
    () => (activeTab === 'pet_types' ? 'pet_type' : ('brand' as const)),
    [activeTab]
  );
  ...
  const {
    product_tags: allTagsList = [],
    count: allTagsCount = 0,
                          ...
                          <Table.HeaderCell>
                            <Checkbox
                              checked={
                                selectedIds.size > 0 &&
                                selectedIds.size ===
                                  tags.filter((tag: CustomTag) => tag.status !== 'pending')
                                    .length &&
                                tags.filter((tag: CustomTag) => tag.status !== 'pending').length > 0
                              }

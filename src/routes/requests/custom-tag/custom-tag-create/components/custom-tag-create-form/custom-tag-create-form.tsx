...
const CustomTagCreateSchema = z.object({
  value: z.string().trim().min(1, 'Tag value is required'),
...
export const CustomTagCreateForm = () => {
  ...
  const defaultType =
    (state as { type?: 'pet_type' | 'brand' } | null)?.type === 'brand' ? 'brand' : 'pet_type';
  ...
  const handleSubmit = form.handleSubmit(async data => {
    try {
      ...
      toast.error(error instanceof Error ? error.message : t('general.somethingWentWrong'));
    }
  });
          ...
          <div className="flex w-full max-w-[720px] flex-col gap-y-8">
            <div className="flex flex-col gap-y-1">
              ...
              <RouteFocusModal.Description asChild>
                <Text className="text-ui-fg-subtle">{t('customTags.create.subtitle')}</Text>

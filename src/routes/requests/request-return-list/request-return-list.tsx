          ...
          <Table.Body data-testid="request-return-list-table-body">
            {requests?.map(request => {
              return (
                <Table.Row
                  key={request.id}
                  data-testid={`request-return-list-table-row-${request.id}`}
                >
                  <Table.Cell data-testid={`request-return-list-table-row-${request.id}-order-id`}>
                    {request.order?.id}
                  </Table.Cell>
                  <Table.Cell
                    data-testid={`request-return-list-table-row-${request.id}-customer`}
                  >{`${request.order?.customer?.first_name} ${request.order?.customer?.last_name}`}</Table.Cell>
                  <Table.Cell data-testid={`request-return-list-table-row-${request.id}-seller`}>
                    {request.seller?.name}
                  </Table.Cell>
                  <Table.Cell data-testid={`request-return-list-table-row-${request.id}-reason`}>
                    {request.customer_note}
                  </Table.Cell>
                  <Table.Cell
                    data-testid={`request-return-list-table-row-${request.id}-escalated-date`}
                  >

...
export const ProductOrganizationForm = ({ product }: ProductOrganizationFormProps) => {
  ...
  const petTypes = useComboboxData<CustomTagsApiResponse, Record<string, string | number>>({
    ...
    queryFn: async params => {
      const response = await sdk.client.fetch<{ tags: CustomTag[]; count: number }>(
        '/admin/custom-tags',
        {
          method: 'GET',
  ...
  const form = useExtendableForm({
    defaultValues: {
      ...
      custom_tag_1: (product.custom_tags || []).find(ct => ct.type === 'pet_type')?.id ?? null,
      custom_tag_2: (product.custom_tags || []).find(ct => ct.type === 'brand')?.id ?? null
    },
    ...
    try {
      ...
      if (hasTagChanges && existingTagIds.length) {
        ...
        const deleteResponse = await fetch(
          `${backendUrl}/admin/products/${product.id}/custom-tags`,
          {
            method: 'DELETE',
            headers: {
          ...
          >
            ...
            <Form.Field
              ...
              render={({ field: { ref, onChange, value, ...field } }) => {
                return (
                  <Form.Item>
                    <Form.Label optional>{t('products.fields.petType.label')}</Form.Label>
                    <Form.Control>
                      <Select
                        {...field}
            ...
            <Form.Field
              ...
              render={({ field: { ref, onChange, value, ...field } }) => {
                return (
                  <Form.Item>
                    <Form.Label optional>{t('products.fields.brand.label')}</Form.Label>
                    <Form.Control>
                      <Select
                        {...field}
                        value={value || undefined}
